...
const AboutSection = (props)=>{
    ...
    return (
        <Section mobileTop>
            ...
            <motion.button onClick={()=> setSection(3)} className="bg-indigo-600 text-white py-4 px-8 roundd-lg font-bold text-lg mt-4 md:mt-16"
                      initial={{
                        opacity:0,
                        y:25,
                    }}
...
const SkillsSection = ()=>{
    return (
      ...
      <motion.div className="w-full" whileInView={"visible"}>
        <h2 className="text-xl md:text-3xl font-bold text-white mt-1">Skills</h2>
        <div className=" mt-4 space-y-1">
          {skills.map((skill, index) => (
            <div className="w-full md:w-64" key={index}>
              <motion.h3
                className="text-l md:text-xl md:font-bold text-gray-100"
                initial={{
                  opacity: 0,
                }}
                variants={{
        ...
        <div>
          <h2 className="text-xl md:text-3xl  font-bold text-white mt-10">Languages</h2>
          <div className=" mt-4 space-y-1">
...
const ContactSection = ()=>{
    const [state, handleSubmit] = useForm("xeqbgayb");
    return (
        <Section>
      <h2 className="text-3xl font-bold">Contact me</h2>
      <div className="mt-8 p-8 rounded-md bg-white bg-opacity-50 w-96 max-w-full">
          {state.succeeded ? (
            <p className="text-gray-900 text-center ">Thankyou for your message!</p>
          ) : (
<form onSubmit={handleSubmit}>
          ...
          <input
            ...
            className="block w-full rounded-md border-0 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 p-3"
          />
            <ValidationError className="mt-1 text-red-600 "
              errors={state.errors}
            />
          <label
            for="email"
